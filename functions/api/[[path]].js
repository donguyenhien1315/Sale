...
function emptyStore(name="Cửa hàng chính"){return {meta:{name,createdAt:now(),version:"5.1.1"},config:{money:{under1000MeansThousands:true},ai:{preview:true},ui:{compactDebt:true}},products:[],expenses:[],suppliers:[],payables:[],budgets:[],cashbox:{opening:0,closing:0,date:"",note:""},ingredients:[{"id": "ing-ca-phe-phin", "name": "Cà phê phin", "purchasePrice": 1085000, "packageQty": 7000, "unit": "g", "unitCost": 155, "stock": 0, "note": "240g pha được 500ml cà phê phin"}, {"id": "ing-ca-phe-may", "name": "Cà phê máy", "purchasePrice": 234000, "packageQty": 1000, "unit": "g", "unitCost": 234, "stock": 0, "note": "Tạm tính gói 1kg; sửa nếu khác"}, {"id": "ing-bot-kem-muoi", "name": "Bột kem muối", "purchasePrice": 80000, "packageQty": 500, "unit": "g", "unitCost": 160, "stock": 0, "note": "Người dùng cung cấp"}, {"id": "ing-sua-dac", "name": "Sữa đặc", "purchasePrice": 450000, "packageQty": 9120, "unit": "ml", "unitCost": 49.3421052632, "stock": 0, "note": "24 hộp x 380ml"}, {"id": "ing-sua-tuoi", "name": "Sữa tươi", "purchasePrice": 35000, "packageQty": 1000, "unit": "ml", "unitCost": 35, "stock": 0, "note": "1 lít giá 35.000"}, {"id": "ing-duong", "name": "Đường", "purchasePrice": 25000, "packageQty": 1000, "unit": "g", "unitCost": 25, "stock": 0, "note": "Tạm tính 25.000/kg; sửa theo thực tế"}],customers:[],debts:[],sales:[],stockReceipts:[],audits:[],transactions:[],snapshots:[],aliases:[]}}
...
function applyAction(root,action,p){
  ...
  switch(action){
    ...
    case "cashbox.set": {st.cashbox={...(st.cashbox||{}),opening:money(p.opening),note:String(p.note||""),date:String(p.date||now().slice(0,10))};tx(st,action,"Cập nhật quỹ tiền mặt");break}
    ...
    case "stockin.create": {
      ...
      for(const l of (p.lines||[])){
        ...
        const pack=qty(l.packSize||pr.packSize)||1;
        const q=qty(l.qty||((qty(l.cases)*pack)+qty(l.units)));if(q<=0)continue;
        const unitCost=money(l.unitCost||pr.purchasePrice||pr.costPrice||pr.cost||pr.unitCost||0);
        const lineCost=money(l.cost||q*unitCost);
        pr.stock=qty(pr.stock)+q;
        if(unitCost>0){pr.costPrice=unitCost;pr.unitCost=unitCost;}
        r.lines.push({productId:pr.id,qty:q,cases:qty(l.cases),units:qty(l.units),packSize:pack,unitCost,cost:lineCost});
        r.totalCost+=lineCost;
      }
      if(!r.lines.length)bad("Phiếu nhập không có mặt hàng");
      r.supplierId=String(p.supplierId||"");r.paymentStatus=String(p.paymentStatus||"paid");r.method=String(p.method||"cash");st.stockReceipts.push(r);if(r.totalCost>0){if(r.paymentStatus==="unpaid")st.payables.push({id:uid(),supplierId:r.supplierId,sourceId:r.id,amount:r.totalCost,balance:r.totalCost,status:"unpaid",createdAt:r.createdAt,note:r.note});else st.expenses.push({id:uid(),type:"stockin",method:r.method,sourceId:r.id,amount:r.totalCost,note:`Nhập kho${r.note?": "+r.note:""}`,includeInProfit:false,paymentStatus:"paid",createdAt:r.createdAt});}
