...
function applyAction(root,action,p){
  ...
  switch(action){
    ...
    case "product.create": {const x={id:uid(),name:String(p.name||"").trim(),category:String(p.category||"Khác"),unit:String(p.unit||"cái"),packSize:qty(p.packSize)||1,costPrice:money(p.costPrice),purchasePackPrice:money(p.costPrice),unitCost:(money(p.costPrice)/(qty(p.packSize)||1)),salePrice:money(p.salePrice),stock:qty(p.stock),minStock:qty(p.minStock),trackStock:true,active:true};if(!x.name)bad("Tên sản phẩm trống");st.products.push(x);tx(st,action,`Thêm sản phẩm ${x.name}`);break}
    case "product.update": {const x=st.products.find(x=>x.id===p.id);if(!x)bad("Không tìm thấy sản phẩm");Object.assign(x,{name:String(p.name||x.name),category:String(p.category||x.category),unit:String(p.unit||x.unit),packSize:qty(p.packSize)||1,costPrice:money(p.costPrice),purchasePackPrice:money(p.costPrice),unitCost:(money(p.costPrice)/(qty(p.packSize)||1)),salePrice:money(p.salePrice),stock:qty(p.stock),minStock:qty(p.minStock)});tx(st,action,`Sửa sản phẩm ${x.name}`);break}
    ...
    case "stockin.create": {
      ...
      for(const l of (p.lines||[])){
        ...
        const packCost=money(l.packCost||l.caseCost||pr.purchasePackPrice||pr.costPrice||pr.purchasePrice||pr.cost||0);
        ...
        if(packCost>0){pr.costPrice=packCost;pr.purchasePackPrice=packCost;pr.unitCost=unitCost;}
