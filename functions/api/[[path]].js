...
function normalizeRoot(raw){
  if(!raw||typeof raw!=="object")bad("Không đọc được dữ liệu gốc từ Supabase",503);
  if(raw?.__nextV4===true&&Array.isArray(raw.stores)){
    if(!raw.stores.length){
      const id=uid();
      return {__nextV4:true,revision:Number(raw.revision)||0,activeStoreId:id,stores:[{id,name:"Cửa hàng chính",createdAt:now(),data:emptyStore()}],persist:raw.persist||{token:"",writtenAt:""},recoveryLocked:true,recoveryReason:"Danh sách cửa hàng trong Supabase đang rỗng"};
    }
    raw.stores=raw.stores.map(s=>({...s,data:ensureStore(s.data,s.name)}));
    if(!raw.stores.some(s=>s.id===raw.activeStoreId))raw.activeStoreId=raw.stores[0].id;
    return raw;
  }
  if(raw?.__multiStore===true&&Array.isArray(raw.stores)){
    if(!raw.stores.length){
      const id=uid();
      return {__nextV4:true,revision:Number(raw.revision)||0,activeStoreId:id,stores:[{id,name:"Cửa hàng chính",createdAt:now(),data:emptyStore()}],persist:raw.persist||{token:"",writtenAt:""},recoveryLocked:true,recoveryReason:"Dữ liệu multi-store cũ đang rỗng"};
    }
    const converted=raw.stores.map(s=>({id:s.id||uid(),name:s.name||"Cửa hàng",createdAt:s.createdAt||now(),data:ensureStore(s.data||{},s.name||"Cửa hàng")}));
    const activeId=converted.some(s=>s.id===raw.activeStoreId)?raw.activeStoreId:converted[0].id;
    return {__nextV4:true,revision:Number(raw.revision)||0,activeStoreId:activeId,stores:converted,persist:raw.persist||{token:"",writtenAt:""}};
  }
  const seed=ensureStore(raw,"Cửa hàng chính");
  const id=uid();
  const locked=(seed.products?.length||0)+(seed.customers?.length||0)+(seed.debts?.length||0)===0;
  return {__nextV4:true,revision:Number(raw.revision)||0,activeStoreId:id,stores:[{id,name:seed.meta?.name||"Cửa hàng chính",createdAt:now(),data:seed}],persist:raw.persist||{token:"",writtenAt:""},recoveryLocked:locked,recoveryReason:locked?"Supabase đang trả về cửa hàng rỗng":""};
}
...
export async function onRequest(context){
  ...
  try{
    if(path==="/api/bootstrap"&&req.method==="GET"){const root=await readRoot(),s=active(root);return json({revision:root.revision||0,storeId:s.id,store:s.data,stores:root.stores.map(x=>({id:x.id,name:x.name})),recoveryLocked:!!root.recoveryLocked,recoveryReason:root.recoveryReason||""})}
    if(path==="/api/recovery/export"&&req.method==="GET"){const root=await readRoot(),s=active(root);return json({format:"cantin-ai-recovery",version:"5.2.2",exportedAt:now(),revision:root.revision||0,storeId:s.id,store:s.data,stores:root.stores.map(x=>({id:x.id,name:x.name}))})}
    if(path==="/api/recovery/status"&&req.method==="GET"){const root=await readRoot(),s=active(root);return json({revision:root.revision||0,recoveryLocked:!!root.recoveryLocked,store:{name:s.name,products:s.data.products?.length||0,customers:s.data.customers?.length||0,debts:s.data.debts?.length||0}})}
    if(path==="/api/recovery/restore"&&req.method==="POST"){
      const body=await req.json();
      const incoming=ensureStore(clone(body.store||{}),"Cửa hàng chính");
      const counts=(incoming.products?.length||0)+(incoming.customers?.length||0)+(incoming.debts?.length||0);
      if(!counts)bad("File khôi phục đang rỗng",400);
      const current=await readRoot();
      const id=current?.activeStoreId||uid();
      const name=incoming.meta?.name||"Cửa hàng chính";
      const previous=current&&current.stores?.length?clone(current):null;
      const root={__nextV4:true,revision:Number(current?.revision)||0,activeStoreId:id,stores:[{id,name,createdAt:now(),data:incoming}],persist:current?.persist||{token:"",writtenAt:""},restoreBackups:Array.isArray(current?.restoreBackups)?current.restoreBackups.slice(-2):[]};
      if(previous){
        root.restoreBackups.push({id:uid(),createdAt:now(),source:String(body.source||"manual"),root:previous});
        root.restoreBackups=root.restoreBackups.slice(-3);
      }
      const saved=await writeRoot(root),s=active(saved);
      return json({ok:true,revision:saved.revision,storeId:s.id,store:s.data,stores:saved.stores.map(x=>({id:x.id,name:x.name}))});
    }
    if(path==="/api/action"&&req.method==="POST"){const body=await req.json(),root=await readRoot();if(root.recoveryLocked)bad("Recovery đang khóa ghi dữ liệu. Hãy khôi phục dữ liệu trước.",423);if(Number(body.revision)!==Number(root.revision||0))bad("Dữ liệu vừa thay đổi ở thiết bị khác. Hãy đồng bộ lại.",409);applyAction(root,body.action,body.payload||{});const saved=await writeRoot(root),s=active(saved);return json({ok:true,revision:saved.revision,storeId:s.id,store:s.data,stores:saved.stores.map(x=>({id:x.id,name:x.name}))})}
