...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==="/api/recovery/restore"&&req.method==="POST"){
      ...
      const root={__nextV4:true,revision:Number(current?.revision)||0,activeStoreId:id,stores:[{id,name,createdAt:now(),data:incoming}],persist:current?.persist||{token:"",writtenAt:""}};
